...
*/
//sudo gcc -o camserver camserver.c -landor -lpvcam -lm -ldl -lpthread -lraw1394 -I/usr/local/pvcam/examples
...
static char * stopDaemon() {
    char * stopResponse;
    stopResponse = "stopping camserver";
    syslog( LOG_INFO, "stopd Received. Stopping camserver...\n");
    exit(EXIT_SUCCESS);
    return stopResponse;
}
...
int main(int argc , char *argv[]) {
    ...
    char rx_buffer[256];  //data rx_buffer of 1K
    char tx_buffer[256];  //data rx_buffer of 1K
    char * response;
	...
	while(TRUE) {
        ...
        for (i = 0; i < max_clients; i++) {
            ...
            if (FD_ISSET( sd , &readfds)) {
                ...
                if ((valread = read( sd , rx_buffer, 255)) == 0) {
                ...
                //Echo back the message that came in
                    ...
                    response = processCommand(rx_buffer);
                    memcpy(tx_buffer, response, strlen(response)+1);
                    send(sd , tx_buffer , strlen(tx_buffer) , 0 );
